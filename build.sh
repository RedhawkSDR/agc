...
if [ "$1" = "rpm" ]; then
    ...
    if [ -e rh.agc.spec ]; then
        mydir=`dirname $0`
        tmpdir=`mktemp -d`
        cp -r ${mydir} ${tmpdir}/rh.agc-2.0.1
        tar czf ${tmpdir}/rh.agc-2.0.1.tar.gz --exclude=".svn" -C ${tmpdir} rh.agc-2.0.1
        rpmbuild -ta ${tmpdir}/rh.agc-2.0.1.tar.gz
        rm -rf $tmpdir
    else
